...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        current_state.epics.insert(id, epic);
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        let epic_option = current_state.epics.get_mut(&epic_id);

        match epic_option {
            Some(epic) => {
                let id = current_state.last_item_id + 1;
                epic.stories.push(id);
                current_state.stories.insert(id, story);
                current_state.last_item_id = id;
                self.database.write_db(&current_state)?;
                Ok(id)
            }
            None => Err(anyhow::anyhow!("Epic ID {} does not exist", epic_id)),
        }
    }
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        let epic_option = current_state.epics.remove(&epic_id);

        match epic_option {
            Some(epic) => {
                // ids.push(epic_id);
                for story_id in epic.stories {
                    current_state.stories.remove(&story_id);
                    // ids.push(story_id);
                }

                // let max_id = ids.iter().max().unwrap();
                // if current_state.last_item_id <= *max_id {
                //     current_state.last_item_id =  ids.iter().min().unwrap() - 1;
                // }
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        let story_option = current_state.stories.get_mut(&story_id);
        match story_option {
            Some(story) => {
                story.status = status;
                self.database.write_db(&current_state)?;
            ...
            None => Err(anyhow::anyhow!("Story ID {} does not exist", story_id)),
        }
    }
}
